...
# 🧱 מבנה כללי של הפרויקט ותפקידי התיקיות

### **Controllers**

ממשק ה-HTTP של המערכת:

* מקבל בקשות מהלקוח
* ממפה DTO → קריאה ל־Service
* מחזיר תגובות HTTP

> שים לב: אין כאן לוגיקה עסקית – רק ניתוב ו־mapping.

---

### **Services**

שכבת הלוגיקה העסקית והיישום:

* מטפלת ביצירה, עדכון וחיפוש מודעות
* מבצעת אינטראקציה עם ה־Repository
* ממפה בין **Domain** ל־**Contracts**
...
* `FileAdsRepository` מטפל בכל פעולות ה־I/O מול הקובץ `Data/ads.json`
* משתמש ב־`IFileLock` למניעת גישה סימולטנית וקריסת קובץ

---

### **Domain**

מכיל ישויות (`Entities`) ו־Value Objects (כגון `Ad`, `Contact`, `Location`)

* האינוואריאנטים העסקיים של כל מודול

---

### **Contracts**

מכיל את ה־DTOs, בקשות (`Requests`) ותגובות (`Responses`)

> מגדיר את פורמט ה־API – ומפריד בין מבנה הנתונים החיצוני (`wire format`) ללוגיקה הפנימית (`domain model`)

---

### **Validation**

שכבת הולידציה באמצעות **FluentValidation**:

* שומרת על תקינות הקלטים
* מונעת לוגיקה לא תקינה מלהגיע לשירותים

---

### **Middleware**

טיפול ב־**concerns חוצי־מערכת**:

* טיפול בשגיאות (Error Handling) ויצירת `ProblemDetails`
* לוגינג
* הרחבות עתידיות לפייפליין של הבקשות

---

### **Data**

דוגמת נתונים בסיסית (`ads.json`) — משמשת כמאגר מקומי פשוט להרצה ולפיתוח.
...
זהו ה־Composition Root — הנקודה שבה נרשמים כל השירותים.

#### רישום שירותים עיקריים

```csharp
builder.Services.AddControllers();                // MVC
builder.Services.AddSwaggerGen();                 // תיעוד API
...
#### רישום יישומים (Implementations)

```csharp
builder.Services.AddSingleton<IFileLock, FileLock>();             // מנגנון נעילה לקובץ
builder.Services.AddSingleton<IAdsRepository, FileAdsRepository>(); // גישה לנתונים
builder.Services.AddScoped<IAdsService, AdsService>();            // לוגיקה עסקית
```

#### סגנון הזרקה

* מבוסס על **Constructor Injection**
* כל Controller מקבל רק את השירותים שהוא נדרש להם

---

 למה הגישה הזאת?

* **פשטות בפיתוח מקומי** — אחסון בקובץ מסיר תלות במסד נתונים חיצוני.
* **הפרדה ברורה של אחריות** — מאפשרת בדיקות יחידה נוחות והחלפת שכבות (למשל מעבר ל-EF Core) בלי שינוי בשכבות אחרות.
* **יישור קו עם best practices של ASP.NET Core**

  * שימוש מובנה ב־DI
  * ניהול HttpClient עם Factory
  * Middleware Pipeline לניהול זרימת הבקשות
* **בטיחות בכתיבה לקובץ** — `IFileLock` מונע התנגשויות ו־corruption.
* **תמיכה בעברית מלאה** —
  `AddJsonOptions(...)` ב־Controllers + `JsonSerializerOptions` עם
  `JavaScriptEncoder.UnsafeRelaxedJsonEscaping` ב־FileAdsRepository
  מבטיחים ייצוא וקריאה תקינה של תווים בעברית.

---

# 🔁 תזרים בקשה טיפוסי (Request → Storage)

1. הבקשה נכנסת ל־Controller → Model נבנה ע"י model binder
2. **FluentValidation** בודק תקינות → במידת הצורך middleware מחזיר `ProblemDetails`
3. Controller קורא ל־`IAdsService` שמבצע לוגיקה עסקית
4. `FileAdsRepository` קורא/כותב את `ads.json` תוך שימוש ב־`IFileLock`
5. ה־Response מוחזר ללקוח, כשה־JSON מוצג בעברית תקינה
...
Service (Business Logic)
        │
        ▼
Repository (FileAdsRepository)
        │
        ▼
File Storage (Data/ads.json)
        │
        ▼
